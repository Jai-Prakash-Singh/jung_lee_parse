...
def main(directory, filename):
    db = MySQLdb.connect("localhost","root","6Tresxcvbhy","jungleesecond")
    cursor = db.cursor()
    ...
    for line in f:
        try:
            ...
            sql_select = """ select task_id from `jungleesecond_data` where `product_url` = "%s" """ %(line_split[15])
            ...
            if task_id:
                sqlentry = """update jungleesecond_data set product_desc = "%s", product_spec= "%s", target = "%s",  selliing_price = "%s", mrp = "%s", color= "%s", size= "%s", dte= "%s", status= "%s"  where task_id = "%s" """
                ...
                sqlentry = """insert into jungleesecond_data ( product_id, product_title, product_title_zovon,  target_link, \
...
def supermain(directory):
    flptrn = "%s/ct_cl_sct_scl_ssct_sscl_br_bl_pl_info.txt" %(directory)
...
if __name__=="__main__":
    ...
    supermain(directory)
    filename = "/home/desktop/working_sites/junglee/junglee_dir/ct_cl_sct_scl_ssct_sscl_br_bl_pl_info.txt"
    main(directory, filename)
