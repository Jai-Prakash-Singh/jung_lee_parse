...
def supermain():
    ...
    sql = """create table IF NOT EXISTS  jungleesecond_data_cmn (
    ...
    try:
        sql2 = "create unique index jungleesecond_data_cmn_index on jungleesecond_data_cmn (Link)"
        cursor.execute(sql2)
        db.commit()   
    except:
        pass
    ...
    try:
        sql2 = "create  index jungleesecond_data_cmn_index2 on jungleesecond_data_cmn \
    ...
    sql = """alter ignore  table  jungleesecond_data_cmn  add   updated_just_now  enum('NO','YES', 'F') DEFAULT 'NO';""" 
