...
class to_common(object):
	...
	db = MySQLdb.connect("xxxx","root","xxxx", dbname)
	cursor = db.cursor()

	self.db = db 
	self.cursor = cursor

        self.db2 = db2 = MySQLdb.connect("xxxx","root","xxxx", "zov", local_infile = 1)
    ...
    def table_connect(self):
        ...
        sql = """insert into %s ( SKU, Title, Link, Price, Category, subCategory, Brand, Image, ListPrice,\
				  ...
				  product_desc, product_spec, seller_brand from %s where  upload_status = 'YES'\
                                  and upload_image_status = "NO" \
    ...
    def to_zovon(self):
        ...
        sql = """ select SKU, Title, Link, Price, Category, subCategory, Brand, Image, ListPrice,\
                  ColourName, targetAudience, productUrl, seller, metaTitle, metaDescription,\
                  size, log_discription, log_specification, seller_brand  from %s  where updated_just_now = "NO" INTO OUTFILE "%s" \
...
def supermain():
    import time 
    dbname, table1, table2, seller_brand = ("junglee_class", "junglee_class_data", "junglee_class_data_cmn", "www.junglee.com")
